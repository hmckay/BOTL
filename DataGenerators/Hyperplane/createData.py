...
def main():
    ...
    if conflicting:
        ...
        print functions
        createDataset(NUM_CONCEPTS,functions,num_attrs,DEPENDANT,"ICMLData/SOURCE"+options.OUT_FP+".csv",NOISE_PROB,schedules,conceptLengths,conflicting,conceptComponents,0,combinedKeys,gradual,driftLen)
        for o in range(1,6):
            if shared > 0:
                ...
                if randOrder:
                    conceptComponents,functions = shuffleComponents(conceptComponents,functions,min(conceptComponents.keys()))
                for k in range(1,6):
                    createDataset(NUM_CONCEPTS,functions,num_attrs,DEPENDANT,"ICMLData/TARGET"+str(o)+options.OUT_FP+str(k)+".csv",NOISE_PROB,schedules,conceptLengths,conflicting,conceptComponents,combinedC,combinedKeys,gradual,driftLen)
            else:
