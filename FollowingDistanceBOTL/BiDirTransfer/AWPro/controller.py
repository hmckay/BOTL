...
# models between them. This implementation uses AWPro as the underlying 
# drift detector.
############################################################################
import numpy as np
import subprocess
...
def initiate(threadID,name,uid,PORT,fp,inFile,outFile,sFrom,sTo,weightType,recievedModels,runID,nums,cullThresh,miThresh):
    ...
    if weightType == 'OLSKPAC2' or weightType == 'OLSPAC' or weightType == 'OLSCL2' or weightType == 'OLSCL':
        out = open(outFile,'w')
    else:
        out = open(os.devnull,'w')
    ...
    args = ['python3',fp,str(threadID),str(PORT),str(sFrom),str(sTo),inFile,str(INIT_DAYS), 
            ...
            str(nums),str(uid),str(cullThresh),str(miThresh),str(MODEL_HIST_THRESHOLD_ACC),str(MODEL_HIST_THRESHOLD_PROB)]
...
def main():
    ...
    MODEL_HIST_THRESHOLD_ACC =float(sys.argv[9])
    MODEL_HIST_THRESHOLD_PROB=float(sys.argv[10])
