...
def runADWIN(df,sourceModels,tLabel,weightType,s):
    ...
    for idx in df.index:
        ...
        # if ofUse >= STABLE_SIZE and not sentFlag and not tempModel:
        ...
        #continue with repro
            ...
            if retrainFlag==False:
                ...
                if len(window) >= MAX_WINDOW:
                    ...
                    # print("len window is too big")
                    partial = window.copy()
                    print("at start"+str(len(partial))+str(tempModel))
                    #retrain model
                    # print("making a real model: "+str(tempModel))
                    sourceModels=readyToReceive(s,sourceModels)
                    modelID, existingModels,transitionMatrix,modelOrder,targetModel = modelMultiHistory.nextModels(existingModels,
                            transitionMatrix,modelOrder,partial,modelID,tLabel,DROP_FIELDS,startIDX,False,weightType,tempModel)
                    # send to controller
                    ofUse = 0
                    sentFlag = 0
                    tempModel = False
                    ...
                    sourceModels=readyToReceive(s,sourceModels)
                    modelID,existingModels,transitionMatrix,modelOrder,model = modelMultiHistory.tempModel(existingModels,transitionMatrix,
                            modelOrder,window,modelID,tLabel,DROP_FIELDS,startIDX,False,tempModel)
            ...
            if updateFlag:
                ...
                if n1 >= MAX_WINDOW:
                    ...
                    modelStart.append(startIDX)
                    conceptSize = len(window)
                    # print("n1 bigger than window len")
                    partial = historyData.iloc[(MAX_WINDOW*(-1)):].copy()
                    # print("building a model on: ")
                    # print(partial)
                    #retrain model
                    print("1: "+str(len(partial))+str(tempModel))
                    sourceModels=readyToReceive(s,sourceModels)
                    modelID, existingModels,transitionMatrix,modelOrder,targetModel = modelMultiHistory.nextModels(existingModels,
                            transitionMatrix,modelOrder,partial,modelID,tLabel,DROP_FIELDS,startIDX,False,weightType,tempModel)
                    tempModel = False
                    ...
                    if lastModID < modelID:
                        drifts[modelCount] = week
                    print("new model and new adwin windows created")
                    retrainFlag=False
                    window = window.iloc[0:0]
                else:
                    window = historyData.iloc[int(n1)*-1:].copy()
                    # print("windowlength is : "+str(len(window)))
                    if modelMultiHistory.reuseModels(existingModels,window, tLabel,DROP_FIELDS):
                        startIDX=idx
                        # print("22: "+str(len(window))+str(tempModel))
                        sourceModels=readyToReceive(s,sourceModels)
                        modelID, existingModels,transitionMatrix,modelOrder,targetModel = modelMultiHistory.nextModels(existingModels,
                                transitionMatrix,modelOrder,window,modelID,tLabel,DROP_FIELDS,startIDX,False,weightType,tempModel)
                        ofUse = 0
                        sentFlag = 0
                        weights = modelMulti.calcWeights(window,sourceModels,targetModel,tLabel,
                                DROP_FIELDS,weightType,CULLTHRESH,MITHRESH,stableLocals,modelID,existingModels[modelID]['PCs'])
                        ...
                        if n1 >= MAX_WINDOW/2:
                            ...
                            sourceModels=readyToReceive(s,sourceModels)
                            modelID, existingModels,transitionMatrix,modelOrder,targetModel = modelMultiHistory.tempModel(existingModels,
                                    transitionMatrix,modelOrder,window,modelID,tLabel,DROP_FIELDS,startIDX,False,tempModel)
                            weights = modelMulti.calcWeights(window,sourceModels,targetModel,tLabel,
                                    DROP_FIELDS,weightType,CULLTHRESH,MITHRESH,stableLocals,modelID,existingModels[modelID]['PCs'])
                            ofUse = 0
                            tempModel = True
...
def main():
    ...
    MODEL_HIST_THRESHOLD_ACC =float(sys.argv[16])
    MODEL_HIST_THRESHOLD_PROB=float(sys.argv[17])
